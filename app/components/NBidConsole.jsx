  ...
  function normaliseChatPayload(result) {
    ...
    const pushMsg = (content) => {
      ...
      if (typeof content === "object") {
        const msg = content.message ?? content.Message ?? content.text ?? content.output;
        if (msg) {
          out.push({
            role: "assistant",
            content: typeof msg === "string" ? msg : JSON.stringify(msg, null, 2),
            parsed: tryParseJSON(msg) ?? content,
          });
        } else {
          out.push({ role: "assistant", content: JSON.stringify(content, null, 2), parsed: content });
        }
      } else {
        out.push({ role: "assistant", content: String(content), parsed: tryParseJSON(content) });
      }
    };
  ...
  async function runWorkflow(e) {
    ...
    try {
      ...
      if (showLogs && res.body) {
        ...
        accumulated = contentType.includes("json")
      ...
      logNet(`Completed in ${(performance.now() - t0Ref.current).toFixed(0)} ms`);
    } catch (err) {
  ...
  const hasCompliance = useMemo(() => {
    for (const r of responses) {
      ...
      if (p && ("compliant" in p || "Compliant" in p) && ("message" in p || "Message" in p)) {
        return { compliant: Boolean(p.compliant ?? p.Compliant), message: String(p.message ?? p.Message ?? r.content) };
      }
    }
    return null;
  }, [responses]);
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <header className="mb-6">
        <h1 className="text-2xl font-bold bg-gradient-to-r from-indigo-400 to-cyan-400 text-transparent bg-clip-text">PropEasy</h1>
        <p className="text-sm opacity-80">Upload two PDFs and watch live logs while the workflow runs.</p>
      </header>

      <form onSubmit={runWorkflow} className="space-y-4 bg-white/5 p-4 rounded-2xl border border-white/10">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-sm">
            <input id="logs" type="checkbox" checked={showLogs} onChange={(e) => setShowLogs(e.target.checked)} />
        ...
        <div className="flex items-center gap-3">
          <button disabled={loading} type="submit" className="rounded-xl px-4 py-2 bg-white/10 border border-white/20 hover:bg-white/20 disabled:opacity-50">Run workflow</button>
          <button type="button" onClick={() => { setResponses([]); setNetLog(""); setLiveLog(""); }} className="rounded-xl px-3 py-2 text-sm border border-white/20 hover:bg-white/10">Clear</button>
      ...
      <section className="mt-6 grid md:grid-cols-2 gap-4">
        ...
        <div className="border border-white/10 rounded-2xl p-3">
          ...
          <pre ref={liveLogRef} className="text-xs bg-black/50 rounded p-2 min-h-[6rem] overflow-auto max-h-64 whitespace-pre-wrap">{liveLog || "—"}</pre>
        </div>
      </section>
              ...
              {Object.entries(writers).map(([bucket, items]) => (
                <div key={bucket} className="border border-white/10 rounded-xl p-3">
                  ...
                  {items.length===0 ? <p className="text-sm opacity-60">No content</p> : (
                    <ul className="space-y-2 text-sm max-h-60 overflow-auto">{items.map((r)=>(<li key={r.id} className="bg-white/5 rounded p-2 whitespace-pre-wrap">{r.content}</li>))}</ul>
                  )}
                </div>
              ))}
            </div>
